import {
  createSociety,
  deleteBilan,
  getBilanById,
  getBilans,
  getMaterials,
  saveCalculation,
  saveMaterials,
} from './carbonazeApi';
import { environment } from '../environment/environment';

describe('carbonazeApi', () => {
  const mockFetch = jest.fn();

  beforeEach(() => {
    jest.useFakeTimers();
    jest.setSystemTime(new Date('2026-03-16T09:15:00.000Z'));
    mockFetch.mockReset();
    global.fetch = mockFetch as typeof fetch;
  });

  afterEach(() => {
    jest.useRealTimers();
  });

  it('reads the materials catalog', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => [{ id: 1, name: 'Acier', energeticValue: 1.9, quantity: 0 }],
    });

    await expect(getMaterials()).resolves.toEqual([
      { id: 1, name: 'Acier', energeticValue: 1.9, quantity: 0 },
    ]);
    expect(mockFetch).toHaveBeenCalledWith(`${environment.apiUrl}/materials`, {
      headers: { Accept: 'application/json' },
    });
  });

  it('creates a society with the default mobile name', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ id: 42, name: 'Carbonaze Mobile' }),
    });

    await expect(createSociety()).resolves.toEqual({ id: 42, name: 'Carbonaze Mobile' });
    expect(mockFetch).toHaveBeenCalledWith(`${environment.apiUrl}/societies`, {
      method: 'POST',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ name: 'Carbonaze Mobile' }),
    });
  });

  it('reads saved bilans', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => [{ id: 3, calculationDate: '2026-03-16', totalCo2: 18.2 }],
    });

    await expect(getBilans()).resolves.toEqual([
      { id: 3, calculationDate: '2026-03-16', totalCo2: 18.2 },
    ]);
    expect(mockFetch).toHaveBeenCalledWith(`${environment.apiUrl}/bilans`, {
      headers: { Accept: 'application/json' },
    });
  });

  it('reads a saved bilan by id', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        id: 7,
        siteId: 3,
        electricityKwhYear: 10000,
        gasKwhYear: 2000,
        totalCo2: 12.4,
        calculationDate: '2026-03-17',
      }),
    });

    await expect(getBilanById(7)).resolves.toEqual({
      id: 7,
      siteId: 3,
      electricityKwhYear: 10000,
      gasKwhYear: 2000,
      totalCo2: 12.4,
      calculationDate: '2026-03-17',
    });
    expect(mockFetch).toHaveBeenCalledWith(`${environment.apiUrl}/bilans/7`, {
      headers: { Accept: 'application/json' },
    });
  });

  it('saves materials through the sync endpoint', async () => {
    const payload = [{ name: 'Bois', energeticValue: 0.08, quantity: 3 }];
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => [{ id: 9, name: 'Bois', energeticValue: 0.08, quantity: 3 }],
    });

    await expect(saveMaterials(payload)).resolves.toEqual([
      { id: 9, name: 'Bois', energeticValue: 0.08, quantity: 3 },
    ]);
    expect(mockFetch).toHaveBeenCalledWith(`${environment.apiUrl}/materials`, {
      method: 'POST',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
      },
      body: JSON.stringify(payload),
    });
  });

  it('creates a site then its bilan with transformed payload values', async () => {
    mockFetch
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ id: 12 }),
      })
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ id: 87, siteId: 12, calculationDate: '2026-03-16' }),
      });

    await expect(
      saveCalculation(
        {
          siteName: 'Site Paris',
          city: 'Paris',
          employees: 12.8,
          parkingSpaces: 7.2,
          computers: 18.5,
          energyMwh: 12.4,
          gasMwh: 7.5,
          totalCo2: 22.7,
        },
        4,
      ),
    ).resolves.toEqual({
      siteId: 12,
      bilanId: 87,
      calculationDate: '2026-03-16',
    });

    expect(mockFetch).toHaveBeenNthCalledWith(1, `${environment.apiUrl}/sites`, {
      method: 'POST',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        name: 'Site Paris',
        city: 'Paris',
        numberEmployee: 13,
        parkingPlaces: 7,
        numberPc: 19,
        societyId: 4,
      }),
    });
    expect(mockFetch).toHaveBeenNthCalledWith(2, `${environment.apiUrl}/sites/12/bilans`, {
      method: 'POST',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        electricityKwhYear: 12400,
        gasKwhYear: 7500,
        totalCo2: 22.7,
        calculationDate: '2026-03-16',
      }),
    });
  });

  it('surfaces the backend response text on failure', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: false,
      status: 500,
      text: async () => 'backend exploded',
    });

    await expect(getMaterials()).rejects.toThrow('backend exploded');
  });

  it('deletes a saved bilan', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      status: 204,
      text: async () => '',
    });

    await expect(deleteBilan(7)).resolves.toBeUndefined();
    expect(mockFetch).toHaveBeenCalledWith(`${environment.apiUrl}/bilans/7`, {
      method: 'DELETE',
      headers: { Accept: 'application/json' },
    });
  });
});
